 ...
 * (Fix: EKG & SpO2 schalten automatisch den Puls/HF frei)
 */
exports.handler = async (event) => {
  ...
  try {
    ...
    // SpO2 Messung (FIX: Aktiviert auch Puls)
    if (/spo2|sättigung/.test(low)) { 
        state.measurements.vitals.SpO2 = true; 
        state.measurements.vitals.Puls = true; // Automatisch Puls via Pleth
    ...
    // EKG (FIX: Aktiviert auch Puls/HF)
    if (/ekg/.test(low)) { 
        // Wir setzen Puls auf sichtbar (HF vom Monitor)
